...
:::info 本文档描述现状实现（v1）
合约权威源是 [`spec/timeline-v2.md`](../spec/timeline-v2.md)。v2 规范中 `Event` 是 Tracks + Events 双层模型中的数据层，`DotRecord` 仅作为 v1 兼容形态保留，新实现请对齐 v2。
:::
...
| inputSource | String | 输入来源（事实不可改）：`ble_ring` / `screen_tap` / `screen_double_tap` / `crown_cw` / `crown_ccw` / `action_button` / `double_tap` / `gesture` / `manual` |
| markerNameSnapshot | String? | 打点时的动作名称快照（防止映射变更导致历史打点语义漂移） |
| mappingVersion | int | 映射规则版本号（0 表示未绑定） |
| heartRateSnapshot | int? | 打点瞬间心率（来自 Watch） |
| speedSnapshot | double? | 打点瞬间速度（来自 Watch） |
